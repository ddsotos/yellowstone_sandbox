"""Convert V2 replays to public pre-play board-columns tensors."""
...
from yellowstone.value_v2_lite import canonical_tensors_v2_lite
    ...
    *,
    ...
    for path in paths:
        ...
        if destination.is_file():
            ...
            continue
        rows = [
            _as_preplay_record(record)
            for game in read_replay_shard(path)
            for record in records_from_replay_v2_lite(game)
        ]
        encoded = [canonical_tensors_v2_lite(record) for record in rows]
    ...
    for path in sorted(output_path.glob("part_*.npz")):
        with np.load(path) as archive:
            ...
            total_records += int(len(archive["target"]))
    ...
    manifest = {
        **board_columns_v2_metadata(preplay=True),
        "status": "complete",
...
def _as_preplay_record(record):
    return replace(
        record,
        state=record.state_before_turn,
